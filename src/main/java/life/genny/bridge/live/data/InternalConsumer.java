...
public class InternalConsumer {
  ...
  public void getBlackLists(String protocol) {
    LOG.warn(
        "New recorded info associated to invalid data this protocol {"
            + protocol
            + "} "
  ...
  public static JsonObject removeKeys(final JsonObject json) {
    if (json.containsKey("token")) json.remove("token"); // do not show the token
    ...
    try {
      ...
      try {
        ...
        if (!incoming.contains("<body>Unauthorized</body>")) {
          ...
          LOG.error(
              "The host service of channel producer tried to accessed an endpoint and gotan"
                  + " unauthorised message potentially from api and the producer hosted in"
                  + " rulesservice");
        }
      } catch (GennyKeycloakException e) {
        LOG.error(
            "The token verification has failed somehow this token was able to penatrate other "
