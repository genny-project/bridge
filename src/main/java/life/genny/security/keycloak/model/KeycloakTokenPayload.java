...
/**
 * KeycloakTokenPayload --- The bearer token will be base64 encoded with a 
 * header body and signature and then joined by a dot '.' 
 *
 * This class will decode the body or extract the claims from a {@link JsonWebToken}
 *
 *
...
public class KeycloakTokenPayload {
    ...
    public KeycloakTokenPayload(JsonWebToken payload){
        ...
        this.roles = new JsonObject(payload.getClaim("realm_access").toString()).getJsonArray("roles").stream()
            .map( d  ->d.toString())
    ...
    /**
     * This static method will return an object of this class type {@link KeycloakTokenPayload}.
     * It is recommended to uses this static method to parse the JWT token to a POJO providing 
     * more type safety.
     *
     *
